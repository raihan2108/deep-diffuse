...
class AttentionModel:
    def __init__(self, options, use_att):
        ...
        self.vertex_size = options['vertex_size']
    ...
    def build_graph(self):
        ...
        with tf.variable_scope('glimpse_net'):
            gl = GlimpseNet(self.options, self.input_nodes, self.input_times)
        ...
        init_loc = tf.random_uniform((self.batch_size, 1), maxval=self.seq_len-self.win_len, minval=0)
        init_glimpse = gl(init_loc)
        lstm_cell = tf.nn.rnn_cell.LSTMCell(self.state_size, activation=tf.nn.tanh)
        self.init_state = lstm_cell.zero_state(self.batch_size, tf.float32)
        inputs = [init_glimpse]
        inputs.extend([0] * (self.options['num_glimpse']))
        outputs, _ = legacy_seq2seq.rnn_decoder(inputs, self.init_state, lstm_cell, loop_function=get_next_input)
        if self.use_att:
            self.output = self.attention(outputs)
        else:
    ...
    def calc_node_loss(self):
        state_reshaped = tf.reshape(self.output, [-1, self.state_size])
...
class GlimpseNet:
    def __init__(self, options, input_node_ph, input_time_ph):
        self.original_size = options['seq_len']
        self.win_size = options['win_len']
        self.batch_size = options['batch_size']
        self.seq_len = options['seq_len']
        self.vertex_size = options['vertex_size']
        self.state_size = options['state_size']
        self.emb_size = options['embedding_size']
        self.sensor_size = self.seq_len * (self.emb_size + 1)

        self.hg_size = options['h_size']
        self.hl_size = options['h_size']
    ...
    def get_glimpse(self, loc):
        ...
        for i in range(0, self.batch_size):
            if loc[i] + self.win_size > self.seq_len:
                ...
                f_s = self.win_size
            t_node = tf.slice(self.input_node_ph[i, :], [loc[i]], [f_s])
            t_time = tf.slice(self.input_time_ph[i, :], [loc[i]], [f_s])
